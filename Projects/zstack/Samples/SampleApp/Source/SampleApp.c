...
afStatus_t SampleApp_SendBroadcastMessage(uint8 *buf, uint8 len);
afStatus_t SampleApp_SendP2PMessage(uint8 *buf, uint8 len);
...
afStatus_t SampleApp_SendBroadcastMessage(uint8 *buf, uint8 len)
{
  return AF_DataRequest(&SampleApp_Periodic_DstAddr,
                        &SampleApp_epDesc,
                        SAMPLEAPP_PERIODIC_CLUSTERID,
                        len,
                        buf,
...
afStatus_t SampleApp_SendP2PMessage(uint8 *buf, uint8 len)
{
  return AF_DataRequest(&SampleApp_P2P_DstAddr,
                        &SampleApp_epDesc,
                        SAMPLEAPP_P2P_CLUSTERID,
                        len,
                        buf,
...
void SerialCMD(mtOSALSerialData_t *UartMsg)
{
  ...
  if (SampleApp_NwkState == DEV_ZB_COORD)
  {
    SampleApp_SendBroadcastMessage(buf, len);
  }
  else if (SampleApp_NwkState == DEV_ROUTER)
  {
    SampleApp_SendP2PMessage(buf, len);
  }
}
